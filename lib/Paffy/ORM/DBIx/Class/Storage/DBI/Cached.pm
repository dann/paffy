...
sub select_single {
    ...
    if ( $attrs->{cached} ) {
        my $cache_key
            = $self->_check_cacheable( $ident, $condition, $attrs->{cached} );
        if ($cache_key) {
            ...
            my ( $rv, $sth, @bind )
                = $self->_select( $ident, $select, $condition, $attrs );
